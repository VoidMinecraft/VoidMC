...
use crate::components::{ClientId, MinecraftEntityId, PlayerReady, Position, PreviousPosition, Rotation};
use crate::network::{NetworkChannels, OutgoingPacket};

pub fn broadcast_position(
    channels: Res<NetworkChannels>,
    moved_query: Query<
        (&ClientId, &MinecraftEntityId, &Position, &PreviousPosition, &Rotation),
        (With<PlayerReady>, Changed<Position>),
    >,
...
pub fn update_previous_positions(
    mut query: Query<(&Position, &mut PreviousPosition), (With<PlayerReady>, Changed<Position>)>,
) {
