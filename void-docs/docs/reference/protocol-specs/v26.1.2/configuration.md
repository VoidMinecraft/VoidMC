...
| ID | Name | Direction |
|----|------|-----------|
| `0x00` | [Cookie Request](#0x00---cookie-request) | Client-bound |
| `0x01` | [Custom Payload (Plugin Message)](#0x01---custom-payload-plugin-message) | Client-bound |
| `0x02` | [Disconnect](#0x02---disconnect) | Client-bound |
| `0x03` | [Finish Configuration](#0x03---finish-configuration-clientbound) | Client-bound |
| `0x04` | [Keep Alive](#0x04---keep-alive) | Client-bound |
| `0x05` | [Ping](#0x05---ping) | Client-bound |
| `0x06` | [Reset Chat](#0x06---reset-chat) | Client-bound |
| `0x07` | [Registry Data](#0x07---registry-data) | Client-bound |
| `0x08` | [Resource Pack Pop](#0x08---resource-pack-pop) | Client-bound |
| `0x09` | [Resource Pack Push](#0x09---resource-pack-push) | Client-bound |
| `0x0A` | [Store Cookie](#0x0a---store-cookie) | Client-bound |
| `0x0B` | [Transfer](#0x0b---transfer) | Client-bound |
| `0x0C` | [Update Enabled Features](#0x0c---update-enabled-features) | Client-bound |
| `0x0D` | [Update Tags](#0x0d---update-tags) | Client-bound |
| `0x0E` | [Select Known Packs](#0x0e---select-known-packs) | Client-bound |
| `0x0F` | [Custom Report Details](#0x0f---custom-report-details) | Client-bound |
| `0x10` | [Server Links](#0x10---server-links) | Client-bound |
| `0x11` | [Clear Dialog](#0x11---clear-dialog) | Client-bound |
| `0x12` | [Show Dialog](#0x12---show-dialog) | Client-bound |
| `0x13` | [Code of Conduct](#0x13---code-of-conduct) | Client-bound |
| `0x00` | [Client Information](#0x00---client-information) | Server-bound |
| `0x01` | [Cookie Response](#0x01---cookie-response) | Server-bound |
| `0x02` | [Custom Payload (Plugin Message)](#0x02---custom-payload-plugin-message) | Server-bound |
| `0x03` | [Finish Configuration](#0x03---finish-configuration-serverbound) | Server-bound |
| `0x04` | [Keep Alive](#0x04---keep-alive-1) | Server-bound |
| `0x05` | [Pong](#0x05---pong) | Server-bound |
| `0x06` | [Resource Pack Response](#0x06---resource-pack-response) | Server-bound |
| `0x07` | [Select Known Packs](#0x07---select-known-packs) | Server-bound |
| `0x08` | [Custom Click Action](#0x08---custom-click-action) | Server-bound |
| `0x09` | [Accept Code of Conduct](#0x09---accept-code-of-conduct) | Server-bound |
...
**Semantics.** Identical in shape and intent to [Login Cookie Request](./login#cookie-request). The client replies with [Cookie Response](#0x01---cookie-response).

### 0x01 - Custom Payload (Plugin Message)
...
### 0x03 - Finish Configuration (clientbound)

**Packet ID:** `0x03` · **State:** Configuration · **Bound To:** Client

This packet has no fields.

**Semantics.** Terminal clientbound packet of the Configuration state. On receipt the client must finish processing all queued Configuration packets, switch its connection state to Play, and acknowledge with [Finish Configuration (serverbound)](#0x03---finish-configuration-serverbound). The server switches to Play upon receiving the acknowledgement.
...
**Semantics.** Sent periodically (vanilla: every 15 s) to detect dead connections. The client must echo the ID back in [Keep Alive (serverbound)](#0x04---keep-alive-1) within 30 s or be disconnected with `disconnect.timeout`.
...
**Semantics.** Latency probe distinct from Keep Alive — the client replies with [Pong](#0x05---pong) but no timeout enforcement is implied.
...
| Entries | [Prefixed Array](./data-types#prefixed-array) | A length-prefixed list of `(Identifier id, Optional<NBT> data)`. When `data` is absent, the entry is a *reference* to a value provided by a known pack agreed upon via [Select Known Packs](#0x0e---select-known-packs); when present, it overrides or defines the entry inline as binary NBT. |
...
**Semantics.** Reverses a previous [Resource Pack Push](#0x09---resource-pack-push). Vanilla supports a stack of layered server packs since 1.20.3.

### 0x09 - Resource Pack Push

**Packet ID:** `0x09` · **State:** Configuration · **Bound To:** Client

| Field | Type | Notes |
|-------|------|-------|
| ID | [UUID](./data-types#uuid) | A unique handle the server can later use in [Resource Pack Pop](#0x08---resource-pack-pop). |
...
**Semantics.** Pushes a new resource pack onto the client's pack stack. The client reports its progress via [Resource Pack Response](#0x06---resource-pack-response) (multiple times — `ACCEPTED`, `DOWNLOADED`, `SUCCESSFULLY_LOADED` / failure terminals).
...
**Semantics.** Asks the client to remember an opaque blob keyed by Identifier. A future [Cookie Request](#0x00---cookie-request) (in either Login or Configuration) returns the same blob. Cookies survive [Transfer](#0x0b---transfer), enabling session-token-style flows across servers.
...
**Semantics.** Instructs the client to disconnect from the current server and reconnect to the specified address with `Next State = 3` (Transfer) in its [Handshake](./handshake#intention-handshake). Cookies stored via [Store Cookie](#0x0a---store-cookie) survive the transfer.
...
**Semantics.** Tells the client which experimental / opt-in feature packs are active so it knows whether to render bundle items, etc. Sent before [Registry Data](#0x07---registry-data) because the registry contents depend on enabled features.
...
**Semantics.** Sent after [Registry Data](#0x07---registry-data). The client uses these to resolve tag references in subsequent packets (block tags, item tags, fluid tags, biome tags, damage-type tags, …). See [Tags](./tags).
...
**Semantics.** Sent before [Registry Data](#0x07---registry-data). Lists the data packs whose contents the *server* assumes the client already has bundled. The client replies with [Select Known Packs (serverbound)](#0x07---select-known-packs) listing the subset it actually recognises; for those packs the server omits the entry data in subsequent Registry Data packets, drastically shrinking the join handshake.
...
**Semantics.** Closes the dialog window opened by the most recent [Show Dialog](#0x12---show-dialog).
...
**Semantics.** Opens a server-defined dialog UI on the client. User interactions are reported back via [Custom Click Action](#0x08---custom-click-action). See the dialog system documentation under [Registries](./registries#dialog).

### 0x13 - Code of Conduct
...
**Semantics.** Asks the client to display a code-of-conduct screen. The client must reply with [Accept Code of Conduct](#0x09---accept-code-of-conduct) before the server will send [Finish Configuration](#0x03---finish-configuration-clientbound).
...
| Field | Type | Notes |
|-------|------|-------|
| Key | [Identifier](./data-types#identifier) | Echoes the key from [Cookie Request](#0x00---cookie-request). |
...
**Semantics.** Reply to [Cookie Request](#0x00---cookie-request). Identical shape to the Login version.

### 0x02 - Custom Payload (Plugin Message)
...
### 0x03 - Finish Configuration (serverbound)

**Packet ID:** `0x03` · **State:** Configuration · **Bound To:** Server

This packet has no fields.

**Semantics.** Acknowledgement of [Finish Configuration (clientbound)](#0x03---finish-configuration-clientbound). Terminal serverbound packet of the Configuration state — both sides switch to Play immediately.
...
| Field | Type | Notes |
|-------|------|-------|
| ID | [Long](./data-types#long) | The exact ID echoed from the most recent [Keep Alive (clientbound)](#0x04---keep-alive). |
...
| Field | Type | Notes |
|-------|------|-------|
| ID | [Int](./data-types#int) | Echoes the ID from [Ping](#0x05---ping). |

**Semantics.** Reply to [Ping](#0x05---ping). Used for latency measurement; not subject to a hard timeout.
...
| Field | Type | Notes |
|-------|------|-------|
| ID | [UUID](./data-types#uuid) | Identifies the [Resource Pack Push](#0x09---resource-pack-push) this status refers to. |
...
**Semantics.** Reply to [Select Known Packs (clientbound)](#0x0e---select-known-packs). Determines which Registry Data entries the server may send by reference instead of inline.
...
**Semantics.** Sent in response to a [Show Dialog](#0x12---show-dialog) when the user clicks a button bound to a custom action. Also reachable from Play.
...
**Semantics.** Sent by the client when the user accepts the code of conduct displayed by [Code of Conduct](#0x13---code-of-conduct). The server should refuse to send [Finish Configuration](#0x03---finish-configuration-clientbound) until this packet has been received (when a code of conduct is configured).
