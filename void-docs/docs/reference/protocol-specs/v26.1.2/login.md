...
| ID | Name | Direction |
|----|------|-----------|
| `0x00` | [Disconnect (Login)](#disconnect-login) | Client-bound |
| `0x01` | [Hello (Encryption Request)](#hello-encryption-request) | Client-bound |
| `0x02` | [Game Profile (Login Success)](#game-profile-login-success) | Client-bound |
| `0x03` | [Login Compression (Set Compression)](#login-compression-set-compression) | Client-bound |
| `0x04` | [Custom Query (Login Plugin Request)](#custom-query-login-plugin-request) | Client-bound |
| `0x05` | [Cookie Request](#cookie-request) | Client-bound |
| `0x00` | [Hello (Login Start)](#hello-login-start) | Server-bound |
| `0x01` | [Key (Encryption Response)](#key-encryption-response) | Server-bound |
| `0x02` | [Custom Query Answer (Login Plugin Response)](#custom-query-answer-login-plugin-response) | Server-bound |
| `0x03` | [Login Acknowledged](#login-acknowledged) | Server-bound |
| `0x04` | [Cookie Response](#cookie-response) | Server-bound |
...
1. **Login Start.** The client sends [Hello (Login Start)](#hello-login-start) with the chosen username and the player's offline-or-cached UUID.
2. **Compression (optional).** If the server has a non-negative compression threshold configured, it sends [Login Compression](#login-compression-set-compression). All subsequent packets — in this state and every later state — are framed using the compressed framing described in the [Overview](./index#framing).
3. **Encryption Request.** In online mode, the server generates a random 4-byte verify token, picks (or reuses) a 1024-bit RSA keypair, and sends [Hello (Encryption Request)](#hello-encryption-request) containing its server-id string, the public key in X.509 SubjectPublicKeyInfo encoding, the verify token, and the `shouldAuthenticate` flag.
...
6. **Login Plugin Query (optional).** Either before or after authentication the server may send any number of [Custom Query](#custom-query-login-plugin-request) packets. Each carries a transaction ID and an opaque payload identified by a namespaced ID; the client replies with a matching [Custom Query Answer](#custom-query-answer-login-plugin-response). Used by Velocity/BungeeCord modern forwarding.
7. **Login Success.** With a profile in hand the server sends [Game Profile (Login Success)](#game-profile-login-success) carrying the full `GameProfile`.
8. **Login Acknowledged.** The client confirms receipt with [Login Acknowledged](#login-acknowledged); both sides switch to [Configuration](./configuration).

If at any step the server needs to abort, it sends [Disconnect (Login)](#disconnect-login) with a [Text Component](./text-component) describing the failure and closes the socket. Cookies (see [Store Cookie](./configuration#store-cookie)) may be requested in the Login state with [Cookie Request](#cookie-request); this is most useful immediately after a [Transfer](./configuration#transfer)-initiated reconnect.
...
| Verify Token | [Byte Array](./data-types#byte-array) ([VarInt](./data-types#varint)-prefixed) | A 4-byte random nonce. The client must echo it back (encrypted) in [Key](#key-encryption-response) so the server can prove the response wasn't replayed. |
...
**Semantics.** Sent after [Hello (Login Start)](#hello-login-start) when the server is in online mode. After this packet the next expected serverbound packet is [Key](#key-encryption-response).

### Game Profile (Login Success)
...
**Semantics.** Terminal client-bound packet of the Login state. The server stops handling Login packets after sending it and waits for [Login Acknowledged](#login-acknowledged).

### Login Compression (Set Compression)
...
**Semantics.** When sent, both sides immediately switch to the compressed framing described in the [Overview](./index#framing). Sent at most once per connection, and only before [Game Profile](#game-profile-login-success).

### Custom Query (Login Plugin Request)

**Packet ID:** `0x04` · **State:** Login · **Bound To:** Client

| Field | Type | Notes |
|-------|------|-------|
| Transaction ID | [VarInt](./data-types#varint) | Unique per outstanding query; the client must echo it in its [Custom Query Answer](#custom-query-answer-login-plugin-response). |
...
**Semantics.** Asks the client to return a previously-stored cookie (see [Store Cookie](./configuration#store-cookie)). The client must reply with [Cookie Response](#cookie-response). Useful immediately after a [Transfer](./configuration#transfer) so the new server can recover the previous server's session token.
...
**Semantics.** First packet of the Login state. Triggers the server to either send [Hello (Encryption Request)](#hello-encryption-request) (online mode) or skip straight to [Login Compression](#login-compression-set-compression) followed by [Game Profile](#game-profile-login-success) (offline mode).
...
| Field | Type | Notes |
|-------|------|-------|
| Shared Secret | [Byte Array](./data-types#byte-array) ([VarInt](./data-types#varint)-prefixed) | The 16-byte AES shared secret, RSA-encrypted with the server's public key from [Hello (Encryption Request)](#hello-encryption-request). |
...
**Semantics.** Concludes the encryption handshake. After receiving and validating it the server keys its AES/CFB8 cipher and either runs the Mojang session check or, if `shouldAuthenticate` was `false`, proceeds straight to [Game Profile](#game-profile-login-success).

### Custom Query Answer (Login Plugin Response)

**Packet ID:** `0x02` · **State:** Login · **Bound To:** Server

| Field | Type | Notes |
|-------|------|-------|
| Transaction ID | [VarInt](./data-types#varint) | Echoes the ID from the corresponding [Custom Query](#custom-query-login-plugin-request). |
...
**Semantics.** Terminal server-bound packet of the Login state. On receipt the server switches its connection state to [Configuration](./configuration); the client must do the same immediately after sending. Sent by the client in response to [Game Profile](#game-profile-login-success).
...
| Field | Type | Notes |
|-------|------|-------|
| Key | [Identifier](./data-types#identifier) | Echoes the key from the [Cookie Request](#cookie-request). |
...
**Semantics.** Sent in reply to [Cookie Request](#cookie-request). The payload is opaque to the client — it merely echoes whatever a server previously stored with [Store Cookie](./configuration#store-cookie).
