...
| ID | Name | Direction |
|----|------|-----------|
| `0x00` | [Status Request](#0x00---status-request) | Server-bound |
| `0x01` | [Ping Request](#0x01---ping-request) | Server-bound |
| `0x00` | [Status Response](#0x00---status-response) | Client-bound |
| `0x01` | [Pong Response](#0x01---pong-response) | Client-bound |
...
**Semantics.** Sent first by the client after entering the Status state. The server replies with exactly one [Status Response](#0x00---status-response). Sending more than one Status Request on the same connection is a protocol error.
...
**Semantics.** Sent by the client after it has received the Status Response. The server immediately replies with [Pong Response](#0x01---pong-response) carrying the same payload, which lets the client compute round-trip latency. The connection is closed by the client after the pong arrives.
...
**Semantics.** Sent in reply to [Status Request](#0x00---status-request). Only one is sent per connection.
...
| Field | Type | Notes |
|-------|------|-------|
| Payload | [Long](./data-types#long) | The exact value received in [Ping Request](#0x01---ping-request). |
